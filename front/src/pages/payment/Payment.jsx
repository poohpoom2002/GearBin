...
import ButtonCash from "../../component/ButtonCash";
import ButtonPromptpay from "../../component/ButtonPromptpay.";
import { Link, useParams, useSearchParams } from "react-router-dom";
import DATA from "../../data/Product_information";

const Payment = () => {
  const param = Number(useParams().key);
  const pirce = useParams().pirce;
  ...
  if (param <= 72) {
    cat = DATA.อาหาร;
  } else if (72 < param && param <= 120) {
    cat = DATA.ขนม;
  } else {
    cat = DATA.เครื่องดื่ม;
  }
  ...
  let path = "";
  if (data.type == "เครื่องดื่มแบบกด") path = "DispenserDrink";
  else if (data.type == "คาเฟ่") path = "DrinkDetail";
  else if (data.type == "อาหารพร้อมทาน") path = "FoodDetail";
  else if (data.type == "อาหารว่างรองท้อง" || data.type == "อาหารแห้ง")
    path = "NormalDetail";
  else if (data.type == "บรรจุขวด") path = "RTDDetail";
  else if (data.type == "เบเกอรี่") path = "SnackDetail";
  else if (data.type == "ขนมและลูกอม") path = "SweetDetail";

  console.log(path, data);
  return (
    //   <MachineZoom yourCode={<Payments price={'39'} name={"ไส้กรอก ต๊อกบอกกีชีส สูตรเผ็ดพ่นไฟ"} pic={"https://7elevenweb.s3.ap-southeast-1.amazonaws.com/product/243fd54c42d14b83a5a0a5055d36ed64.jpg"}/>}/>
    <MachineZoom
      yourCode={
        <div className="w-full h-full bg-white relative">
          <div className="bg-bg-green w-full h-1/6 flex items-center justify-between px-9">
            <Link
              to={`${data.type == "คาเฟ่" ? "../" : ""}../${path}/${param}`}
            >
              <svg
            ...
            <h1 className="text-white text-5xl font-bold flex-grow text-end mr-10 ">
              เลือกวิธีชำระเงิน
            </h1>
          ...
          <div class="w-full mt-4 flex-col flex justify-center items-center">
            <img
              class="w-1/4 h-1/4 mt-0 rounded-[20px]"
              src={data.url}
              alt="image"
            />
            <p className="text-center text-2xl flex-grow  mt-7">
              {data.productName}
            </p>
            <h1 className="text-bg-green text-center text-5xl flex-grow mt-1">
              {`฿${
                discount
                  ? pirce
                    ? Math.floor(pirce * 0.9)
            ...
            {/* <img
              class="w-1/4 h-1/4 mt-0 rounded-[20px]"
              src="https://7elevenweb.s3.ap-southeast-1.amazonaws.com/product/243fd54c42d14b83a5a0a5055d36ed64.jpg"
              alt="image"
            />
            <p className="text-center text-2xl flex-grow  mt-7">
              ไส้กรอก ต๊อกบอกกีชีส สูตรเผ็ดพ่นไฟ
            </p>
            <h1 className="text-bg-green text-center text-5xl flex-grow mt-1">
              ฿39
            </h1> */}
          ...
          <div className="w-full h-[150px]  flex mt-14  ">
            ...
            />
            <ButtonCoupon setDiscount={setDiscount} />
          </div>
